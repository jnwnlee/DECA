...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        ### dump dict to json
        import pickle
        # pickle.dump(testdata[i], open(os.path.join(savefolder, name, 'data_'+str(i)+'.pkl'), 'wb'))
        # torch.save(codedict, os.path.join(savefolder, name, 'codedict' +'.pt'))
        # torch.save(opdict, os.path.join(savefolder, name, 'opdict' +'.pt'))
        # torch.save(visdict, os.path.join(savefolder, name, 'visdict' +'.pt'))

        outputdict = {
            'imagename': testdata[i]['imagename'],
            'original_image': testdata[i]['original_image'],
            'image': testdata[i]['image'],
            'tform': testdata[i]['tform'],
            'pose': codedict['pose'][0],
            'cam': codedict['cam'][0],
            'landmarks2d': opdict['landmarks2d'][0],
            'landmarks3d': opdict['landmarks3d'][0],
            'landmarks3d_world': opdict['landmarks3d_world'][0],
            'shape_image':  visdict['shape_images'][0],
            'shape_detail_image':  visdict['shape_detail_images'][0]
        }
        pickle.dump(outputdict, open(os.path.join(savefolder, name, 'outputdict' +'.pkl'), 'wb'))
